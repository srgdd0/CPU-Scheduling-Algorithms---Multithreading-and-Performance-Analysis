...
class Program
{
    ...
    class TaskInfo
    {
        ...
        public int Priority { get; set; } // The priority of the task (higher priority means the task should be executed first)
    }
    ...
    static void Main(string[] args)
    {
        ...
        for (int i = 0; i < numThreads; i++)
        {
            ...
            taskInfos.Add(new TaskInfo
            {
                ...
                Priority = numThreads - i // Assign a priority to the task (higher priority means the task should be executed first)
            });
        }

        // Sort the tasks by their priorities (highest priority first)
        taskInfos = taskInfos.OrderByDescending(t => t.Priority).ToList();
